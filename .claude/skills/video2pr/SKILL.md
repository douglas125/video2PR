...
## Phase 7.5: Offer to Create GitHub Issues (optional)

After presenting remaining tasks and before starting implementation, ask: "Would you like to create GitHub Issues for any of these tasks?"

If the user declines or `gh` is not installed/authenticated, skip this phase.

If the user agrees:
1. For each task selected by the user, show a preview of the issue:
   - **Title**: task title from plan.md
   - **Body**: approach, priority, complexity, affected files, and transcript excerpt
2. **Wait for explicit user confirmation** before creating each issue — never batch-create without per-issue approval.
3. Create the issue:
   ```bash
...
5. Ask "Create next issue?" before proceeding to the next one.
